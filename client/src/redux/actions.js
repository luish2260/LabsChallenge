...
import { OBTENER_PRODUCTOS, SIGUIENTES_PRODUCTOS, ANTERIORES_PRODUCTOS, PARA_FILTRAR_USADOS, PARA_FILTRAR_NUEVOS, SIGUIENTES_PRODUCTOS_FILTRADOS, ANTERIORES_PRODUCTOS_FILTRADOS, PARA_FILTRAR_MENOR_PRECIO, PARA_FILTRAR_MAYOR_PRECIO } from './const_reducers.js';
// OBTENER PRODUCTOS
export const obtenerProductos = (valor) => async (dispatch, getState) => {
  try {
    const res = await axios.get('http://localhost:5000/api/search/' + valor + '&limit=30'); /* + '&limit=30' */
    dispatch({
...
export const siguientesProductos = (valor) => async (dispatch, getState) => {
  ...
  try {
    const res = await axios.get('http://localhost:5000/api/search/' + valor + '&offset=' + siguientes + '&limit=30'); /* + '&limit=30' */
    dispatch({
...
export const anterioresProductos = (valor) => async (dispatch, getState) => {
  ...
  try {
    const res = await axios.get('http://localhost:5000/api/search/' + valor + '&offset=' + anteriores + '&limit=30'); /* + '&limit=30' */
    dispatch({
